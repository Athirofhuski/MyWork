...
import com.example.foodyapp.Util.HouseItem
...
//        fetchDataFromDatabase()
        if (GlobalBox.selectedHouseItem != null ){ //check GlobalBox not null
            var data = GlobalBox.selectedHouseItem

            val point = Point(data!!.long, data!!.lat, SpatialReferences.getWgs84())
...
*/
    private fun fetchDataFromDatabase() {
        ...
        dialog.show()
        houseItemList.clear()
        firebaseRepository.queryDataFromDatabase().get().addOnSuccessListener { documents ->
            dialog.dismiss()
